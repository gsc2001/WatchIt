...
export abstract class VMManager {
    ...
    public getAdjustedBuffer = () => {
        ...
        const rampDownHours =
            config.VM_POOL_RAMP_DOWN_HOURS.split(',').map(Number);
    ...
    public getRedisHostCacheKey = () => {
        return (
            'hostCache' + this.id + this.region + (this.isLarge ? 'Large' : '')
        );
    };

    public getRedisPoolSizeKey = () => {
        return (
            'vmPoolFull' + this.id + this.region + (this.isLarge ? 'Large' : '')
        );
    };
    ...
    public runBackgroundJobs = async () => {
        try {
            ...
            const resizeVMGroupIncr = async () => {
                const availableCount = await this.redis.llen(
                    this.getRedisQueueKey()
                );
                const stagingCount = await this.redis.llen(
            ...
            const cleanupVMGroup = async () => {
                ...
                for (let i = 0; i < allVMs.length; i++) {
                    if (
                        await this.redis.get(`lock:${this.id}:${allVMs[i].id}`)
                    ) {
            ...
            setImmediate(async () => {
                while (true) {
                    ...
                    await new Promise(resolve =>
                        setTimeout(resolve, cleanupInterval)
                    );
                }
            });
