...
export class Room {
    ...
    constructor(
        ...
        io.of(roomId).on('connection', (socket: Socket) => {
            ...
            socket.on('CMD:addReaction', data =>
    ...
    private getHostState = (): HostState => {
        ...
        const match = this.roster.find(
            user =>
                this.clientIdMap[user.id] === this.vBrowser?.controllerClient
        );
        return {
            video: this.video ?? '',
            videoTS: this.videoTS,
    ...
    private playlistNext = (socket: Socket | null, data?: string) => {
        ...
        const votes = this.roster.filter(
            user => this.nextVotes[user.id]
        ).length;
    ...
    private startVBrowser = async (
        ...
        if (redis) {
            ...
            if (uid) {
                ...
                const uidMinutes = await redis.zincrby(
                    'vBrowserUIDMinutes',
                    1,
                    uid
                );
