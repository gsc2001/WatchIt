...
app.get('/searchSubtitles', async (req, res) => {
    try {
        ...
        if (url) {
            ...
            const size = Number(
                startResp.headers['content-range'].split('/')[1]
            );
...
app.get('/health/:metric', async (req, res) => {
    ...
    const result =
        vmManagerStats[req.params.metric]?.availableVBrowsers?.length;
...
app.delete('/deleteAccount', async (req, res) => {
    ...
    if (postgres) {
        // Delete rooms
        await postgres?.query('DELETE FROM room WHERE owner = $1', [
            decoded.uid,
        ]);
...
async function minuteMetrics() {
    ...
    for (let i = 0; i < roomArr.length; i++) {
        ...
        if (room.vBrowser && room.vBrowser.id) {
            ...
            await redis?.expire(
                'vBrowserUIDLock:' + room.vBrowser?.creatorUID,
                120
            );
...
async function getStats() {
    ...
    const numPermaRooms = Number(
        (
            await postgres?.query(
                'SELECT count(1) from room WHERE owner IS NOT NULL'
            )
        )?.rows[0].count
    );
    const numSubs = Number(
        (await postgres?.query('SELECT count(1) from subscriber'))?.rows[0]
            .count
    );
    ...
    const vBrowserStopManual = await getRedisCountDay(
